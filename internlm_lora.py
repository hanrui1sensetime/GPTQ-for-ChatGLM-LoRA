...
def get_internlm(model):
    ...
    peft_path = "/root/workspace/external_data/internlm-7b/lora"
...
@torch.no_grad()
def internlm_sequential(model, dataloader, dev):
    print('Starting ...')
...
@torch.no_grad()
def internlm_eval(model, testenc, dev):
    print('Evaluating ...')
...
def internlm_pack(model, quantizers, wbits, groupsize):
...
def load_quant(model, checkpoint, wbits, groupsize=-1, fused_mlp=True, eval=True, warmup_autotune=True, act_order=True):
    from transformers import modeling_utils, AutoConfig, AutoModelForCausalLM
...
def internlm_multigpu(model, gpus, gpu_dist):
...
if __name__ == '__main__':
    ...
    parser.add_argument('model', type=str, help='internlm model to load')
    ...
    if not args.load and args.wbits < 16 and not args.nearest:
        tick = time.time()
        quantizers = internlm_sequential(model, dataloader, DEV)
        print(time.time() - tick)

    if args.benchmark:
        gpus = [torch.device('cuda:%d' % i) for i in range(torch.cuda.device_count())]
        if len(gpus) > 1:
            internlm_multigpu(model, gpus, gpu_dist)
        else:
            model = model.to(DEV)
        if args.benchmark:
    ...
    if args.eval:
        ...
        for dataset in datasets:
            ...
            print(dataset)
            internlm_eval(model, testloader, DEV)

    if args.quant_directory is not None:
        export_quant_table(quantizers, args.quant_directory)

    if not args.observe and args.save:
        internlm_pack(model, quantizers, args.wbits, args.groupsize)
