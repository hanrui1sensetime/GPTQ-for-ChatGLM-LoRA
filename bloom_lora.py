...
@torch.no_grad()
def bloom_sequential(model, dataloader, dev):
    print('Starting ...')
    ...
    for i in range(len(layers)):
        ...
        if args.true_sequential:
            sequential = [['self_attention.query_key_value'], ['self_attention.dense'], ['mlp.dense_h_to_4h'], ['mlp.dense_4h_to_h']]
        else:
...
def bloom_multigpu(model, gpus, gpu_dist):
    model.transformer.word_embeddings = model.transformer.word_embeddings.to(gpus[0])
    model.transformer.word_embeddings_layernorm = model.transformer.word_embeddings_layernorm.to(gpus[-1])
    import copy
...
if __name__ == '__main__':
    ...
    parser.add_argument('model', type=str, help='bloom model to load')
    ...
    if args.benchmark:
        ...
        if len(gpus) > 1:
            bloom_multigpu(model, gpus, gpu_dist)
        else:
            model = model.to(DEV)
        if args.benchmark:
